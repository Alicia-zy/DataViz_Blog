...
This is the same competition as in 2014 and we will review those winning projects before doing the analysis. 
...
ggplot(ccdata_count,aes(x=reorder(cleaned,n), y=n)) +
  ...
  theme_classic()+
  geom_text(aes(label = n),hjust=1,colour = "white",size=8)+
  theme(plot.title = element_text(hjust = 0.5))
```
...
![](image/Network_1.png){height=500}

![](image/megaMan.png){height=500}

![](image/POK.png){height=500}

![](image/KronosStar.png){height=500}
